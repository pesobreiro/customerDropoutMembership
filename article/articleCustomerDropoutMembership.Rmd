...
performance in the prediction of the dropout such the usage of clusters
combined with churn prediction [@hung_applying_2006; @gok_case_2015;
...
```{python createModel, eval=TRUE, echo=FALSE, cache = TRUE, warning = FALSE}
from pysurvival.models.survival_forest import RandomSurvivalForestModel

# Number of samples in the dataset
N = df_members.shape[0]

# Building training and testing sets
from sklearn.model_selection import train_test_split
index_train, index_test = train_test_split( range(N), test_size = 0.4)
data_train = df_members.loc[index_train].reset_index( drop = True )
data_test  = df_members.loc[index_test].reset_index( drop = True )

# Creating the X, T and E inputs
X_train, X_test = df_members[features], data_test[features]
T_train, T_test = df_members[time_column], data_test[time_column]
E_train, E_test = df_members[event_column], data_test[event_column]
...
```{python pysurvivalperformance, eval=TRUE, echo=FALSE, cache=FALSE, warning = FALSE}
...
The calculation of he number of clusters used the package mclust
[@scrucca2016] using the Bayesian Information Criterion (BIC). The model that
gives the minimum BIC score can be selected as the best model [@schwarz1978]
simplifying the problem related to choosing the number of components and
identifying the structure of the covariance matrix, based on modelling with
multivariate normal distributions for each component that forms the data set
[@akogul2016].
...
Estou a ter problemas com o cálculo dos clusters com o BIC... 
estava a aqui a confirmar os clusters, talvez seja melhor reduzir as variáveis...
testar a abordagem aos clusters no artigo dos vinhos...
...
```{python elbowCalculation, eval=TRUE, echo=FALSE, cache=TRUE, warning = FALSE}
...
TODO: Estava aqui... fit the model for the five clusters... compare the performance with the model without
clusters... corrigir acima para selecionar só as features

```{python createModelClusters, eval=FALSE, echo=FALSE, cache = TRUE, warning = FALSE}
# Building training and testing sets for the clusters
for cluster in df_members.cluster.unique():
    # Number of samples in the dataset
    df_members_cluster = df_members[df_members.cluster == cluster]
    N = df_members_cluster.shape[0]
    index_train, index_test = train_test_split(range(N), test_size = 0.4)
    data_train = df_members_cluster[features].loc[index_train].reset_index( drop = True )
    data_test  = df_members_cluster[features].loc[index_test].reset_index( drop = True )

    # Creating the X, T and E inputs
    X_train, X_test = df_members_cluster[features], data_test[features]
    T_train, T_test = df_members_cluster[time_column], data_test[time_column]
    E_train, E_test = df_members_cluster[event_column], data_test[event_column]
...
strategies [@coussement_churn_2008]. Some assumptions that underlie the adoption
of uplift metrics consider that customers with a higher risk of churning could
not be the best targets, as suggested by @ascarza_retention_2018. Other
researchers addressed the problem using the top-decile lift to develop more
proactive actions to retain the customers at risk of churning
...
best values of these parameters for every model are listed in (ref:table1).
